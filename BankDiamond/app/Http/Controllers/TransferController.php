...
class TransferController extends Controller
{
    public function sendFiles()
    {
        ...
        if($kwotaExpress > 0) {
            ...
            $transfer->nadawca = '89132421540000000000000003';
            $transfer->odbiorca = '78102052260000000000001000';
            $transfer->nazwa_nad = 'Diamond Holdings';
       ...
       if($transfers == null && $returnTrueTransfers == null && $returnFalseTransfers == null) {
        ...
        foreach ($transfers as $t) {
          //  $finalDate = $t['created_at'];
            ...
            array_push($finalTransfers, ['Sender' =>['Name' => $t['nazwa_nad'],'Account' => $t['nadawca'],'Address'=> $t['adres_nad'],'ZIP'=>$t['kod_pocztowy_nad'],'Town'=>$t['miejscowosc_nad'],'BankName' => 'Diamond Holdings'],'Recipient'=>['Name' => $t['nazwa_odb'],'Account' => $t['odbiorca'],'Address'=> $t['adres_odb'],'ZIP'=>$t['kod_pocztowy_odb'],'Town'=>$t['miejscowosc_odb'],'BankName' => ''],'Details'=>['Timestamp'=>$finalDate,'Title'=>$t['tytul'],'Amount'=>$t['kwota']]]);
        }
        foreach($returnTrueTransfers as $t) {
            array_push($returnFinalTransfers, ['Sender' =>['Name' => $t['nazwa_nad'],'Account' => $t['nadawca'],'Address'=> $t['adres_nad'],'ZIP'=>$t['kod_pocztowy_nad'],'Town'=>$t['miejscowosc_nad'],'BankName' => 'Diamond Holdings'],'Recipient'=>['Name' => $t['nazwa_odb'],'Account' => $t['odbiorca'],'Address'=> $t['adres_odb'],'ZIP'=>$t['kod_pocztowy_odb'],'Town'=>$t['miejscowosc_odb'],'BankName' => ''],'Details'=>['Timestamp'=>$t['created_at'],'Title'=>$t['tytul'],'Amount'=>$t['kwota'],'Executed'=>'true']]);
        }
        foreach($returnFalseTransfers as $t) {
            $amount = $amount + $t['kwota'];
            array_push($returnFinalTransfers, ['Sender' =>['Name' => $t['nazwa_nad'],'Account' => $t['nadawca'],'Address'=> $t['adres_nad'],'ZIP'=>$t['kod_pocztowy_nad'],'Town'=>$t['miejscowosc_nad'],'BankName' => 'Diamond Holdings'],'Recipient'=>['Name' => $t['nazwa_odb'],'Account' => $t['odbiorca'],'Address'=> $t['adres_odb'],'ZIP'=>$t['kod_pocztowy_odb'],'Town'=>$t['miejscowosc_odb'],'BankName' => ''],'Details'=>['Timestamp'=>$t['created_at'],'Title'=>$t['tytul'],'Amount'=>$t['kwota'],'Executed'=>'false']]);
        }
        $array = ['BankData' => ['Name'=> 'Diamond Holdings','AccountNumber' => '73132421540000000000000000', 'Amount' => $amount], 'Transfer' => $finalTransfers, 'ReturnTransfer'=>$returnFinalTransfers];
    ...
    public function expressTransfer(Request $request)
    {
       ...
       $req = Http::withBody($json,'application/json')->post('https://bankamethyst.herokuapp.com/api/amethyst/expressAdin');
         if($req == true) {
            app('App\Http\Controllers\AccountController')->changeBalance($nadawca, $kwota*(-1));
